...
PetiteVue.createApp({
  ...
  get filtList() {
    ...
    //   //今ここ
    // }
    if (this.displayfilter == "displayPart") {
      return this.studentList.filter((el) => el.joined == true);
    }
    if (this.classfilter == "classA") {
      console.log(
        this.studentList.filter((el) => el.class == "A" && el.joined == true)
      );
      return this.studentList.filter(
        (el) => el.class == "A" && el.joined == true
      );
    }
    if (this.classfilter == "classB") {
      return this.studentList.filter(
        (el) => el.class == "B" && el.joined == true
      );
    }
    if (this.classfilter == "classC") {
      return this.studentList.filter(
        (el) => el.class == "C" && el.joined == true
      );
    }
    if (this.classfilter == "classD") {
      return this.studentList.filter(
        (el) => el.class == "D" && el.joined == true
      );
    } else {
  ...
  clickJoin(i) {
    ...
    if (this.studentList[i].joined == false) {
      this.studentList[i].style = {
        backgroundColor: "#d6eded",
        boxShadow: "0 3px 0 #35aeae inset",
      };
      this.studentList[i].joined = true;
    } else {
      this.studentList[i].style = {
        backgroundColor: "#ffffff",
        boxShadow: "0px 2px 0 #35aeae",
      };
      this.studentList[i].joined = false;
    }
  },
  get joinList() {
    if (this.classfilter == "classA") {
